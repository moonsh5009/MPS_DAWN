...
std::string_view PDSpringTermProvider::GetTermName() const {
    return "PDSpringTermProvider";
}

bool PDSpringTermProvider::HasConfig(const Database& db, Entity entity) const {
    return db.HasComponent<ext_dynamics::SpringConstraintData>(entity);
}

std::unique_ptr<IProjectiveTerm> PDSpringTermProvider::CreateTerm(
    const Database& db, Entity entity, uint32 /* node_count */) {

    const auto* config = db.GetComponent<ext_dynamics::SpringConstraintData>(entity);
    if (!config) {
        LogError("PDSpringTermProvider: no SpringConstraintData on entity ", entity);
        return nullptr;
    }
    ...
    std::vector<ext_dynamics::SpringEdge> all_edges;
    ...
    if (scoped) {
        ...
        for (Entity mesh_e : entities) {
            ...
            if (count == 0) continue;
            const auto* data = static_cast<const ext_dynamics::SpringEdge*>(storage->GetArrayData(mesh_e));
            ...
            for (uint32 i = 0; i < count; ++i) {
                ext_dynamics::SpringEdge g = data[i];
                g.n0 += node_offset;
                g.n1 += node_offset;
    ...
    return std::make_unique<PDSpringTerm>(all_edges, config->stiffness);
}

void PDSpringTermProvider::DeclareTopology(uint32& out_edge_count, uint32& out_face_count) {
    out_edge_count = edge_count_;
    out_face_count = 0;
}

void PDSpringTermProvider::QueryTopology(const Database& db, Entity entity,
