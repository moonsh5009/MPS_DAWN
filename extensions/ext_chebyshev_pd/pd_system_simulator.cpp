...
const std::string ChebyshevPDSystemSimulator::kName = "ChebyshevPDSystemSimulator";
...
ChebyshevPDSystemSimulator::ChebyshevPDSystemSimulator(system::System& system)
    : system_(system) {}

ChebyshevPDSystemSimulator::~ChebyshevPDSystemSimulator() = default;

const std::string& ChebyshevPDSystemSimulator::GetName() const {
    return kName;
}

void ChebyshevPDSystemSimulator::Initialize() {
    const auto& db = system_.GetDatabase();

    // Find ChebyshevPDSystemConfig entities
    auto config_type_id = GetComponentTypeId<ChebyshevPDSystemConfig>();
    ...
    if (!config_storage_base || config_storage_base->GetDenseCount() == 0) {
        LogInfo("ChebyshevPDSystemSimulator: no ChebyshevPDSystemConfig entities found, skipping");
        return;
    }

    const auto* config_storage =
        static_cast<const ComponentStorage<ChebyshevPDSystemConfig>*>(config_storage_base);
    const auto& config_entities = config_storage->GetEntities();

    Entity config_entity = config_entities[0];
    const auto* config = db.GetComponent<ChebyshevPDSystemConfig>(config_entity);
    if (!config) return;
    ...
    if (config->mesh_entity != database::kInvalidEntity) {
        ...
        if (!pos_entry) {
            LogError("ChebyshevPDSystemSimulator: no SimPosition array entry");
            return;
        }
        node_offset_ = pos_entry->GetEntityOffset(mesh_entity_);
        if (node_offset_ == UINT32_MAX) {
            LogError("ChebyshevPDSystemSimulator: mesh entity ", mesh_entity_, " not in SimPosition");
            return;
        }
        ...
        if (node_count_ == 0) {
            LogError("ChebyshevPDSystemSimulator: mesh entity has 0 SimPosition nodes");
            return;
        }
        ...
        if (node_count_ == 0) {
            LogError("ChebyshevPDSystemSimulator: no SimPosition entities found");
            return;
        }
    ...
    for (uint32 i = 0; i < config->constraint_count; ++i) {
        ...
        for (auto* provider : providers) {
            ...
            if (term) {
                LogInfo("ChebyshevPDSystemSimulator: added term '", term->GetName(),
    ...
    initialized_ = true;
    LogInfo("ChebyshevPDSystemSimulator: initialized (", node_count_, " nodes)");
}

void ChebyshevPDSystemSimulator::Update() {
...
ChebyshevPDSystemSimulator::TopologySignature
ChebyshevPDSystemSimulator::ComputeTopologySignature() const {
    TopologySignature sig;
    sig.node_count = system_.GetArrayTotalCount<SimPosition>();

    const auto& db = system_.GetDatabase();
    auto* storage = db.GetStorageById(GetComponentTypeId<ChebyshevPDSystemConfig>());
    if (!storage || storage->GetDenseCount() == 0) return sig;

    auto* typed = static_cast<const ComponentStorage<ChebyshevPDSystemConfig>*>(storage);
...
void ChebyshevPDSystemSimulator::OnDatabaseChanged() {
    ...
    LogInfo("ChebyshevPDSystemSimulator: topology changed, reinitializing...");
    Shutdown();
    topology_sig_ = new_sig;
    Initialize();
}

void ChebyshevPDSystemSimulator::Shutdown() {
    ...
    LogInfo("ChebyshevPDSystemSimulator: shutdown");
}
