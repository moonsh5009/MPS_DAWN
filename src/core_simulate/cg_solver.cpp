...
void CGSolver::CreateBuffers() {
    ...
    cg_z_ = std::make_unique<GPUBuffer<float32>>(BufferConfig{.usage = srw, .size = vec_sz, .label = "cg_z"});
...
void CGSolver::CreatePipelines() {
    ...
    cg_precond_pipeline_ = MakePipeline("cg_precond.wgsl", "cg_precond");
}
...
void CGSolver::CacheBindGroups(WGPUBuffer physics_buffer, uint64 physics_size,
                               ...
                               ISpMVOperator& spmv,
                               WGPUBuffer diag_buffer, uint64 diag_size) {
    ...
    WGPUBuffer z_h = cg_z_->GetHandle();
    ...
    // PCG bind groups (Jacobi preconditioning)
    has_preconditioner_ = (diag_buffer != nullptr);
    if (has_preconditioner_) {
        bg_precond_ = MakeBG(cg_precond_pipeline_, "bg_cg_precond",
            {{0, {params_buffer, params_size}},
             {1, {r_h, vec_sz}}, {2, {z_h, vec_sz}},
             {3, {diag_buffer, diag_size}}, {4, {mass_buffer, mass_size}}});

        // dot(r, z) for rz computation (reuses cg_dot shader)
        bg_dot_rz_ = MakeBG(cg_dot_pipeline_, "bg_dot_rz",
            {{0, {params_buffer, params_size}},
             {1, {r_h, vec_sz}}, {2, {z_h, vec_sz}}, {3, {partial_h, partial_sz}}});

        // p = z + beta * p (binds z where r normally goes)
        bg_p_pcg_ = MakeBG(cg_update_p_pipeline_, "bg_p_pcg",
            {{0, {params_buffer, params_size}},
             {1, {z_h, vec_sz}}, {2, {p_h, vec_sz}},
             {3, {scalar_h, scalar_sz}}, {4, {mass_buffer, mass_size}}});

        LogInfo("CGSolver: PCG bind groups cached (Jacobi preconditioner)");
    }
...
void CGSolver::Solve(WGPUCommandEncoder encoder, uint32 cg_iterations) {
    ...
    // CG init: x = 0, filter r for pinned, p = r (or p = z for PCG)
    Dispatch(encoder, cg_init_pipeline_, bg_init_, workgroup_count_);

    if (has_preconditioner_) {
        // PCG path: z = D^{-1} * r, then p = z
        Dispatch(encoder, cg_precond_pipeline_, bg_precond_, workgroup_count_);
        wgpuCommandEncoderCopyBufferToBuffer(encoder,
            cg_z_->GetHandle(), 0, cg_p_->GetHandle(), 0, vec_sz);

        // Initial rz = dot(r, z) -> scalars[0]
        Dispatch(encoder, cg_dot_pipeline_, bg_dot_rz_, workgroup_count_);
        Dispatch(encoder, cg_dot_final_pipeline_, bg_df_rr_, 1);
    } else {
        // Standard CG: initial rr = dot(r, r) -> scalars[0]
    ...
    for (uint32 cit = 0; cit < cg_iterations; ++cit) {
        ...
        // alpha = rr / pAp -> scalars[3]  (or rz / pAp for PCG)
        Dispatch(encoder, cg_compute_scalars_pipeline_, bg_alpha_, 1);

        // x += alpha*p, r -= alpha*Ap (MPCG filter zeros r for pinned)
        ...
        if (has_preconditioner_) {
            ...
            // rz_new = dot(r, z) -> scalars[2]
            Dispatch(encoder, cg_dot_pipeline_, bg_dot_rz_, workgroup_count_);
            Dispatch(encoder, cg_dot_final_pipeline_, bg_df_rr_new_, 1);
        } else {
            // Standard CG: rr_new = dot(r, r) -> scalars[2]
            Dispatch(encoder, cg_dot_pipeline_, bg_dot_rr_, workgroup_count_);
            Dispatch(encoder, cg_dot_final_pipeline_, bg_df_rr_new_, 1);
        }
        ...
        if (has_preconditioner_) {
            // PCG: p = z + beta * p (MPCG filter zeros p for pinned)
            Dispatch(encoder, cg_update_p_pipeline_, bg_p_pcg_, workgroup_count_);
        } else {
            // Standard CG: p = r + beta * p
