...
//
// Reads partial sums from level-1 reduction and writes the final
// result to scalars[target_slot].
...
fn cs_main(
    ...
    // Each thread accumulates a strided portion of the partials
    var sum = 0.0;
    var i = local_id;
    loop {
        if (i >= count) {
    ...
    // Write final result to the target scalar slot
    if (local_id == 0u) {
