...
//
// Each face contributes its (unnormalized) face normal to all three vertices.
// Normals are accumulated as fixed-point i32 atomics and later normalized
// by compute_normals_normalize.
...
@group(0) @binding(2) var<storage, read> faces: array<ClothFace>;
...
fn cs_main(@builtin(global_invocation_id) gid: vec3u) {
    ...
    // Face normal (area-weighted, unnormalized)
    let e1 = p1 - p0;
    let e2 = p2 - p0;
    ...
    // Scatter to each vertex (4 i32 per node: x, y, z, w)
